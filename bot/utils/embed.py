...
from bot.utils.db import get_items, get_metadata


async def build_embed(category: str, name: str) -> Embed | None:
    """Build a Discord embeded message.

    Builds an embeded Discord message based
    on category choice and item name.
    ...
    Returns:
        Embed | None: The embeded Discord messsage,
    ...
    containers = "**Case/Package(s):** None\n"
    ...
    match category:
        case "agents" | "cases" | "packages" | "skins":
    ...
    if category in ("agents", "skins"):
        match items.grade:
            ...
            case "Distinguished" | "Mil-Spec":
                embed.color = Color("#4B69CD")
            case "Exceptional" | "Restricted":
                embed.color = Color("#8847FF")
            case "Superior" | "Classified":
    ...
    if category in ("cases", "collections", "packages"):
